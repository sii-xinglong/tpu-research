...
def chunk_parallel_delta_attention(
  ...
  # Flatten for efficient vmap
  k_c_flat = key_c.reshape(-1, chunk_size, k_head_dim)
  g_c_flat = g_cumsum.reshape(-1, chunk_size, k_head_dim)
  beta_c_flat = beta_c.reshape(-1, chunk_size)
  v_c_flat = value_c.reshape(-1, chunk_size, v_head_dim)
  
  def compute_chunk_vars(k_blk, g_blk, beta_blk, v_blk):
      prec = jax.lax.Precision.HIGHEST

      # Stable A_raw calculation using broadcasting
  ...
  compute_vmap = jax.vmap(compute_chunk_vars)
  u_c_flat, w_c_flat = compute_vmap(k_c_flat, g_c_flat, beta_c_flat, v_c_flat)
  
  u_c = u_c_flat.reshape(batch_size, num_heads, num_chunks, chunk_size, v_head_dim)
  w_c = w_c_flat.reshape(batch_size, num_heads, num_chunks, chunk_size, k_head_dim)
