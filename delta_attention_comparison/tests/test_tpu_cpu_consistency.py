...
def test_tpu_cpu_consistency(B_list=[1, 2], L_list=[1024, 2048, 4096], H=2304, NH=32, NV=32, D=128, K=4):
    ...
    # JIT for CPU and TPU
    step_cpu = jax.jit(grad_fn, backend='cpu')
    step_tpu = jax.jit(grad_fn, backend='tpu')
    ...
    for B in B_list:
        for L in L_list:
            ...
            # Run on CPU
            try:
                (loss_cpu, dx_cpu) = step_cpu(model, x, dy)
            ...
            # Run on TPU
            try:
                (loss_tpu, dx_tpu) = step_tpu(model, x, dy)
